...
from electroncash.bitcoin import Hash, SignatureType, hash_160, serialize_xpub, var_int
...
class Satochip_KeyStore(Hardware_KeyStore):
    ...
    def sign_transaction(self, tx: Transaction, password, *, use_cache=False):
        ...
        self.print_error(
            "sign_transaction(): hashOutputs= ", hashOutputs
        )  # debugSatochip
        ...
        for i, txin in enumerate(tx.inputs()):
            ...
            self.print_error(
                "sign_transaction(): input[type]:", txin["type"]
            )  # debugSatochip
            ...
            for j, x_pubkey in enumerate(x_pubkeys):
                self.print_error(
                    "sign_transaction(): forforloop: j=", j
                )  # debugSatochip
                ...
                if x_pubkey in derivations:
                    ...
                    self.print_error(
                        "sign_transaction(): pre_tx_hex=", pre_tx_hex
                    )  # debugSatochip
                    self.print_error(
                        "sign_transaction(): pre_hash=", pre_hash_hex
                    )  # debugSatochip
...
class SatochipPlugin(HW_PluginBase):
    ...
    def setup_device(self, device_info, wizard, purpose):
        ...
        while client.cc.card_present:
            ...
            self.print_error(
                "setup_device(): authentikey=", hex_authentikey
            )  # debugSatochip
