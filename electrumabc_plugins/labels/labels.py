...
class LabelsPlugin(BasePlugin):
    ...
    def set_label(self, wallet, item, label):
        ...
        with wallet.lock:  # need to hold the lock from get nonce to set nonce in order to prevent races.
            ...
            t = threading.Thread(
                target=self.do_request, args=["POST", "/label", False, bundle, True]
            )
            t.setDaemon(True)
    ...
    def _curthr_pop(self):  # misnomer. it's not a stack.
        try:
            ...
            pass  # we silently ignore unbalanced _curthr_push/pop for now...

    def push_thread(self, wallet):
        if wallet not in self.wallets or self.closing:
            return  # still has race conditions here
        try:
            self._curthr_push()
        ...
        try:
            ...
            try:
                ...
                with wallet.lock:
                    ...
                    self.print_error(
                        "received %d labels" % len(response.get("labels", 0))
                    )
                ...
                if force:
                    raise e  # force download means we were in "settings" mode.. notify gui of failure.
        finally:
            self._curthr_pop()
    ...
    def start_wallet(self, wallet):
        ...
        if wallet in self.wallets:
            self.print_error(
                "Wallet", basename, "already in wallets list, aborting early."
            )
            return
        ...
        if not mpk:
            ...
            self.print_error(
                "Wallet",
                basename,
                "is incompatible (no master public key), aborting early.",
            )
            return
        nonce = self.get_nonce(wallet)
